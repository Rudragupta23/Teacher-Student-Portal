...
export default function ParentDashboard() {
  ...
  const handleFeedbackSubmit = async (e) => {
    ...
    try {
      ...
      setFeedbackForm({ feature: 'Child\'s Progress', message: '', rating: 0 });
      setHoveredStar(0);
    } catch (err) {
  ...
  const fetchProfile = async () => {
    try {
      ...
      setParentProfile({ name: res.data.name, profilePic: res.data.profilePic || '', authProvider: res.data.authProvider || 'local' });
  ...
  return (
    <div className="flex h-screen bg-[#F4F7FE] font-sans overflow-hidden text-slate-800 relative">
      ...
      <aside className={`w-72 bg-[#0B1437] text-slate-300 flex flex-col shadow-2xl z-50 fixed inset-y-0 left-0 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:relative lg:translate-x-0 transition-transform duration-300 lg:flex rounded-r-[2rem] my-4 lg:ml-4 overflow-hidden`}>
        <div 
          onClick={() => { navigate('/parent-dashboard/settings'); setIsSidebarOpen(false); }}
        ...
        >
          <div className="relative">
            {parentProfile.profilePic ? (
              <img src={parentProfile.profilePic} alt="Profile" className="w-12 h-12 rounded-2xl object-cover shadow-lg shadow-violet-500/30 group-hover:opacity-75 transition-opacity" />
            ) : (
              <div className="bg-gradient-to-tr from-violet-400 to-indigo-500 text-white w-12 h-12 flex items-center justify-center rounded-2xl font-black text-2xl shadow-lg shadow-violet-500/30 group-hover:opacity-75 transition-opacity">
          ...
          <div>
            <h1 className="text-lg font-black text-white tracking-wide leading-tight group-hover:text-violet-400 transition-colors">MathCom<br/>Mentors</h1>
            
            {!parentProfile.profilePic && (
              <p className="text-[10px] text-violet-400 font-bold mt-1 group-hover:underline">Click to upload your photo</p>
            )}
          </div>
        </div>
  ...
  <button className="w-full sm:w-auto px-8 py-4 bg-[#1B2559] hover:bg-violet-600 text-white font-black rounded-2xl transition-all shadow-lg transform hover:-translate-y-1">Send</button>
          ...
          {activeTab === 'settings' && (
            <div className="animate-fade-in max-w-2xl mx-auto">
                ...
                <form onSubmit={handleProfileUpdate} className="space-y-8">
                  <div className="flex items-center gap-6">
                    ...
                    <div className="flex flex-col items-start gap-2">
                      ...
                      <label className="bg-violet-50 text-violet-700 border-2 border-violet-200 hover:bg-violet-100 px-4 py-2 rounded-xl text-xs font-black cursor-pointer transition-colors shadow-sm inline-flex items-center gap-2 mt-1">
              ...
              {parentProfile.authProvider !== 'google' ? (
                ...
                <div className="bg-violet-50 border border-violet-100 p-6 rounded-[2rem] mt-8 text-center">
                  <p className="text-violet-900 font-bold text-sm">
              ...
              <div className="bg-white p-8 rounded-[2rem] shadow-[0_18px_40px_rgba(112,144,176,0.12)] mt-8 relative overflow-hidden">
                ...
                <form onSubmit={handleFeedbackSubmit} className="space-y-6 flex flex-col h-full relative z-10">
                  <p className="text-sm font-bold text-slate-500">Help us improve the MathCom Mentors portal! Select a specific feature below, give it a rating, and leave your thoughts.</p>
                    ...
                    <div className="flex flex-col sm:flex-row gap-6">
                      ...
                      <div className="space-y-2 flex-1">
                        ...
                        <select className="w-full p-4 bg-white border border-slate-200 rounded-2xl outline-none font-bold text-[#1B2559] focus:ring-4 focus:ring-sky-500/20 transition-all cursor-pointer shadow-sm"
                          ...
                          <optgroup label="Parent Dashboard Tabs">
                            <option value="Child's Progress">📊 Child's Progress</option>
                            ...
                            <option value="Class Calendar">🗓️ Class Calendar</option>
                            <option value="Message Teacher">💬 Message Teacher</option>
                          </optgroup>
                      ...
                      <tr key={report._id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-5">
                          {report.classStatus === 'Class Rescheduled' && report.rescheduledDate ? (
                            ...
                            <div>
                              <p className={`text-sm font-bold px-2 py-1 rounded-md inline-block mb-1 border ${report.classStatus === "Student didn't attend" ? 'text-rose-700 bg-rose-50 border-rose-100' : 'text-violet-700 bg-violet-50 border-violet-100'}`}>
